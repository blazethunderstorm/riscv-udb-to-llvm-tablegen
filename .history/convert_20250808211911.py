...
def parse_assembly_operands(assembly_str, inst):
    ...
    for part in parts:
        if part.startswith('v') and inst == 'V':
            if part == 'vd':
...
def convert_instruction(udb_data):
    ...
    inst = get_instruction_format(udb_data)
    format_data = udb_data.get('format', {})
    ...
    outputs, inputs = parse_assembly_operands(assembly, inst)
...
def {tablegen_name} : RISCVInst<
  ...
  let Format = {inst}Format;{constraint_str}
}}'''
