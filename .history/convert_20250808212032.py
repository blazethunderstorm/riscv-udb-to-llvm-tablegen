...
def convert_instruction(udb_data):
    ...
    output5, inp = parse_assembly_operands(assembly, inst_for)
    
    if not output5 and not inp:
    ...
    outputs_str = ', '.join(output5)
    inputs_str = ', '.join(inp)
    
    asm_parts = []
    if output5:
        asm_parts.extend([f"${op.split(':$')[1]}" for op in output5])
    if inp:
