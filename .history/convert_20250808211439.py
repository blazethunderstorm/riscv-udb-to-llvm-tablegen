...
def get_instruction_format(udb_data):
    ...
    if '$inherits' in format_data:
        ...
        for fmt in ['R', 'I', 'S', 'B', 'U', 'J']:
            if f'/{fmt}/' in inherit_path or f'{fmt}-' in inherit_path:
                return fmt
    
    if 'encoding' in udb_data:
        enc = udb_data['encoding']
        if 'match' in enc and len(enc['match'].replace('-', '')) == 16:
            return 'C'
...
def parse_assembly_operands(assembly_str, inst_format):
    ...
    for part in parts:
        if part.startswith('v') and inst_format == 'V':
            ...
            imm_map = {
                'I': 'simm12:$imm', 'S': 'simm12:$imm', 'B': 'simm13_lsb0:$imm',
                'U': 'uimm20:$imm', 'J': 'simm21_lsb0:$imm'
            }
            inputs.append(imm_map.get(inst_format, 'simm12:$imm'))
...
def convert_instruction(udb_data):
    ...
    return f'''def {tablegen_name} : RISCVInst<
    (outs {outputs_str}),
    (ins {inputs_str}),
    "{name}", "{asm_format}",
    []> {{
...
def convert_csr(udb_data):
    ...
    return f'''def {tablegen_name} : RISCVReg<{address}, "{name}">;
'''
...
def convert_extension(udb_data):
    ...
    return f'''// Extension: {name} - {long_name}
'''
...
def generate_tablegen_header():
    return '''include "RISCVInstrFormats.td"

def GPR : RegisterClass<"RISCV", [i32], 32, (add (sequence "X%u", 0, 31))>;
def VR : RegisterClass<"RISCV", [v64i1, v128i1, v256i1, v512i1, v1024i1], 32, (add (sequence "V%u", 0, 31))>;
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 convert.py input.yaml output.td")
        print("   or: python3 convert.py input_dir/ output_dir/")
        sys.exit(1)
    ...
    if input_path.is_file():
        ...
        if all_content:
            ...
            print(f"Generated {output_file} with {len(all_content)} items")
